...
Scatterplots can powerful in exploring bivariate relationships in an intuitive, graphical form.
Use correlation to quantify strenght of a linear realtinoship between two variables using a single number.
Use linear regression to model repsponse variable in terms of a single explanatory variable
Learned to interpret the coefficients of regression models
...
As you fit a regression model, there are some quantities (e.g. R2) that apply to the model as a whole, while others apply to each observation (e.g. ŷ i). If there are several of these per-observation quantities, it is sometimes convenient to attach them to the original data as new variables.
...
Going back to the textbook example, by looking at residuals we can tell if certain if a certain book is under or over-priced in terms of what the model would predict. The residual is the difference between the prediction and the actual value.  
...
RMSE - root mean squared error - also, residual standard error - RSE  
R displays this value when you get the summary of the the mod object  
Example - if the value is 3.57 for mod_possum, that is saying that the model generally predicts a value within 3.57cm from the truth.  
...
# Compute the mean of the residuals - should be equal to 0
mean(residuals(mod))
...
### How do we evaluate the fit of the model that is unitless?
y (hat) is the predicted value of y  
y (bar) is the average value of y
**the null average model is: y(hat) = y(bar)**  
R^2 is the coefficient of determination - proportion of the variability in the response variable that is explained by the model
Explains the quality of the fit of a regression model  
> "Essentially all models are wrong, but some are useful" - Gearge Box  
...
**The  R^2  represents the percentage of the variability of the response variable that can be explained by the explanatory variable.**  
...
nontrivial_players <- mlbBat10 %>% filter(AB >= 10, OBP<.5)
...
# Rank high leverage, but lowest cooks distance points
mod_cleaner %>%
  augment() %>%
